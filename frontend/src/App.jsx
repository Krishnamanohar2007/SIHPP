...
export default function App(){
  ...
  return <div className="min-h-screen bg-slate-100 lg:grid lg:grid-cols-[16rem_minmax(0,1fr)]">
    <header className="sticky top-0 z-30 flex items-center justify-between bg-slate-950 px-5 py-4 text-white lg:hidden"><div><p className="text-[10px] font-bold uppercase tracking-[.18em] text-emerald-400">SIH 2026 · PS 26017</p><h1 className="text-base font-bold">LandRisk Monitor</h1></div><button aria-label="Toggle navigation" onClick={()=>setOpen(!open)} className="rounded-lg border border-slate-700 px-3 py-2 text-sm">{open?'Close':'Menu'}</button></header>
    {open&&<button aria-label="Close navigation" onClick={()=>setOpen(false)} className="fixed inset-0 z-30 bg-slate-950/50 lg:hidden"/>}
    <aside className={`fixed inset-y-0 left-0 z-40 w-72 bg-slate-950 p-5 text-slate-300 transition-transform lg:sticky lg:top-0 lg:block lg:h-screen lg:w-auto lg:translate-x-0 ${open?'translate-x-0':'-translate-x-full'}`}><div className="mb-8"><p className="text-xs font-bold uppercase tracking-[.2em] text-emerald-400">SIH 2026 · PS 26017</p><h1 className="mt-2 text-lg font-bold text-white">LandRisk Monitor</h1></div><Navigation onNavigate={()=>setOpen(false)}/><p className="mt-8 text-xs leading-5 text-slate-500">Decision support for land acquisition risk and schedule exposure.</p></aside>
